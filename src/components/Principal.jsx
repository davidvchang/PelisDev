...
function Principal({search}) {
  ...
  //       // Búsqueda por categoría (si search es un array de objetos)
  //       return search.some(category => category.id === movie.category);
  //     } else {
  //       // Búsqueda por categoría (si search es un objeto)
  //       return movie.category === search.id;
  //     }
  //   })
  // : Data;

  // Filtrado combinado por título y categoría
  const filteredData = search ? Data.filter(movie => {
    const matchesSearch = typeof search === 'string' ? movie.title.toLowerCase().includes(search.toLowerCase()) : true;
    const matchesCategory = Array.isArray(search) ? search.some(category => category.id === movie.category) : true;
  ...
  return (
    ...
    <section className='w-full min-h-screen flex flex-col justify-center items-center font-Poppins text-base pt-16'>
        ...
        {/* <Link to='/page/5' className='w-10 h-10 bg-slate-300 rounded-md flex justify-center items-center hover:bg-blue-500 hover:transition-all duration-300'>
            <span>5</span>
        </Link> */}
